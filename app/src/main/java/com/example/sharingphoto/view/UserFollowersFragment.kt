...
class UserFollowersFragment : Fragment() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        auth = Firebase.auth
        viewModel = ViewModelProvider(this)[UserFollowersViewModel::class.java]
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            info = UserFollowersFragmentArgs.fromBundle(it).userid
            if(currentUser != null)
            {
                ...
                if(currentUserId != null)
                {
                    lifecycleScope.launch(Dispatchers.IO) {
                        viewModel.getFollowersById(currentUserId!!,info!!)
                    }
                    getAdapter()
                    observerLiveData()
