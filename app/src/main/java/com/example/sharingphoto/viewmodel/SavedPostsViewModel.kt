...
class SavedPostsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val savedPostLiveData = MutableLiveData<List<Post>>()
    ...
    fun getSavedPosts() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try
            {
                ...
                feedRepository.getSavedPost(currentUserId!!).collect {
                        postList ->
                    withContext(Dispatchers.Main) {
                        savedPostLiveData.value = postList
                    }
                }
    ...
    suspend fun removeFromSaved(currentUserId : String,postId : String)
    {
        firestore.collection("Users").document(currentUserId)
            .update("savedPost", FieldValue.arrayRemove(postId)).await()
        getSavedPosts()
    }
