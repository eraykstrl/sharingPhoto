...
class UserFollowersViewModel(application: Application) : AndroidViewModel(application) {
    ...
    suspend fun getFollowersById(currentUserId : String,profileId : String)
    {
        try
        {
            userRepository.getUserFollowersById(currentUserId,profileId).collect {
                list ->
    ...
    suspend fun userOperations(currentUserId: String?,profileId : String?,operationInfo : Int)
    {
        if(currentUserId != null && profileId != null)
        {
            if(operationInfo == 0)
            {
                ...
                getFollowersById(currentUserId,profileId)
            }
