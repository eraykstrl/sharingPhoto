...
class VideoProfileViewModel(application: Application) : AndroidViewModel(application) {
    ...
    suspend fun getVideosFromInternet(userId : String)
    {
        ...
        try
        {
            userRepository.getPostsByUserIdVideo(userId).collect {
                    list ->
                withContext(Dispatchers.Main) {
