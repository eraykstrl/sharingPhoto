...
class DiscoverViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val searchLiveData = MutableLiveData<List<Triple<String, String,String?>>>()
    ...
    fun getPosts() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try
            {
                ...
                feedRepository.getAllPosts(currentUserId!!).collect {
                    postList ->
    ...
    {
        val user = firestore.collection("Users").orderBy("username").startAt(username).endAt(username + "\uf8ff").get().await()
        println("user ${user}")
        searchList.clear()
        for(u in user)
        {
            val new = u.getString("username")
            val user_id = u.getString("user_id")
            val profile = u.getString("profilePhoto")


            searchList.add(Triple(user_id!!,new!!,profile))


        }

        withContext(Dispatchers.Main) {
            searchLiveData.value = searchList
        }
    }
